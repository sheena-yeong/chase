...
app.post("/gemini", async (req, res) => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `You are an assistant that ONLY outputs refined Slack chaser messages; rewrite the draft message "${message}" so it is a light-hearted, slightly sarcastic reminder for the assignee to complete their task, output only the final message with no quotes or explanations, always sound like a chaser even if the input is nonsense, and keep it under 200 characters.`,
    });
